...
const MunicipalElection = () => {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      if (!user.village_id) {
        console.error("User has no village_id assigned");
        setError(
  ...
  const fetchElectionData = async () => {
    try {
      ...
      // Filter lists and candidates based on user's village for mayoral/municipal elections
  ...
  const handleVote = async () => {
    ...
    try {
      ...
      const res = await VOTE_USER_API.post(
        `/mayoral/${election_id}`,
        voteData,
        {
          ...
          {process.env.NODE_ENV === "development" && (
            <div className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
              ...
              <p className="text-yellow-400 text-sm">
                User Village ID: {userVillageId}
              </p>
              <p className="text-yellow-400 text-sm">
                Total Lists: {electionData?.lists?.length || 0}
              </p>
              <p className="text-yellow-400 text-sm">
                Lists:{" "}
                {JSON.stringify(
                  electionData?.lists?.map((l) => ({
                    name: l.list_name,
                    village_id: l.village_id,
                    candidates: l.candidates?.length,
                  })) || [],
                )}
              </p>
            </div>
                    ...
                    <div className={`bg-linear-to-r p-6`}>
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() =>
                              handleSelectAllFromList(list.list_id)
                            }
                        ...
                        <div
                          ...
                          className={`p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 ${
                            selectedCandidates.includes(candidate.candidate_id)
                              ? "border-[#9D5CFF] bg-[#6C2BD9] bg-opacity-10"
