...
const AdminElections = () => {
  ...
  const handleCreateSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await CANDIDATE_ADMIN_API.post("/", formData, {
  ...
  const handleUpdateSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await CANDIDATE_ADMIN_API.put(
        `${selectedElection.id}`,
        formData,
        {
          headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    try {
      const res = await CANDIDATE_ADMIN_API.delete(`/${selectedElection.id}`);
                      ...
                      {filteredElections.map((election) => (
                        ...
                        >
                          ...
                          <td className='px-4 py-3'>
                            <div className='text-[#CCCCCC]'>
                              {election.village && (
                                <div>{election.village}</div>
                              )}
                              {election.district && (
                                <div>{election.district}</div>
                              )}
                              {election.governorate && (
                                <div>{election.governorate}</div>
                              )}
                            </div>
                          </td>
                          <td className='px-4 py-3'>
